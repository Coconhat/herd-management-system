...
export function ComprehensivePrintView({
  ...
  const data = generateComprehensivePrintData({
    records,
    animals,
    viewMode: "table",
  });
    ...
    <div id={id} className="hidden print:block print:p-4">
      ...
      <div className="text-center border-b pb-4 mb-6">
        <h1 className="text-3xl font-bold mb-2">
          Comprehensive Milk Production Report
        </h1>
        ...
        <div className="text-sm text-gray-500">
          ...
          <p>
            Data Period: {data.statistics.dateRange.earliest} -{" "}
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="bg-gray-50 p-4 rounded border">
            <div className="text-2xl font-bold text-blue-600">
              {data.statistics.totalProduction.toFixed(1)} L
            </div>
            <div className="text-sm text-gray-600">Total Production</div>
          </div>
          <div className="bg-gray-50 p-4 rounded border">
            <div className="text-2xl font-bold text-green-600">
              {data.statistics.activeAnimals}
            </div>
          ...
          <div className="bg-gray-50 p-4 rounded border">
            <div className="text-2xl font-bold text-orange-600">
              {data.statistics.averagePerRecord.toFixed(1)} L
            </div>
          ...
          <tbody>
            {data.animalData.map((item, index) => (
              ...
              >
                ...
                <td className="border border-gray-300 px-3 py-2 text-right font-mono">
                  {item.averageProduction.toFixed(1)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {data.animalData.map((animalData, animalIndex) => (
        <div
          key={animalData.animal.id}
          className={`mb-8 ${animalIndex > 0 ? "page-break" : ""}`}
        >
          <h3 className="text-lg font-bold mb-3 flex items-center justify-between">
            <span>
              {animalData.animal.ear_tag} -{" "}
              {animalData.animal.name || "Unnamed"}
            </span>
            <span className="text-sm font-normal text-gray-600">
              Total: {animalData.totalProduction.toFixed(1)}L | Avg:{" "}
              {animalData.averageProduction.toFixed(1)}L
            </span>
          </h3>
          ...
          <table className="w-full border-collapse mb-4">
            ...
            <tbody>
              {animalData.records.map((record, recordIndex) => (
                <tr
                  key={record.id}
                  className={recordIndex % 2 === 0 ? "bg-gray-50" : ""}
                >
                  <td className="border border-gray-300 px-3 py-2">
      ...
      <div className="print:mt-8 print:pt-4 print:border-t border-gray-300 text-center text-xs text-gray-500">
        <p className="mb-1">
          Calving Management System - Comprehensive Milk Production Report
        </p>
        <p>
          Generated automatically on{" "}
          {format(new Date(), "MMMM d, yyyy 'at' h:mm a")} | Please verify data
          accuracy
        </p>
        <p className="mt-2 font-medium">
          Report includes {data.statistics.totalRecords} records from{" "}
          {data.statistics.activeAnimals} animals
        </p>
      </div>
    </div>
  );
